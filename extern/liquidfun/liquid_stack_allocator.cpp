...
b2LiquidStackAllocator::b2LiquidStackAllocator()
{
	m_index = 0;
	m_allocation = 0;
	m_maxAllocation = 0;
...
void* b2LiquidStackAllocator::Allocate(int32 size)
{
...
void* b2LiquidStackAllocator::Reallocate(void* p, int32 size)
{
	b2Assert(m_entryCount > 0);
...
void b2LiquidStackAllocator::Free(void* p)
{
	b2Assert(m_entryCount > 0);
	b2StackEntry* entry = m_entries + m_entryCount - 1;
	b2Assert(p == entry->data);
...
int32 b2LiquidStackAllocator::GetMaxAllocation() const
{
